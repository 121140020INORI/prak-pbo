...
class Robot:
  ...
  def lakukan_aksi(self, enemy):
    ...
    if enemy.health <= 0:
      print()
      if self == myrobot:
        print(f"Robotmu yaitu ({self.nama}) mengalahkan robot lawan :) ({enemy.nama})")
      else:
        print(f"Robot lawan yaitu ({self.nama}) mengalahkan robotmu :( ({enemy.nama})")
